...
describe("dispatchReplyFromConfig", () => {
  ...
  it("delivers plain Telegram tagged block replies with partial suppressed", async () => {
    setNoAbort();
    ttsMocks.state.statusSnapshot = {
      autoMode: "tagged",
      provider: "auto",
    ...
    const replyResolver = async (
      ...
      await opts?.onPartialReply?.({ text: "Plain tagged mode text." });
  ...
  it("tagged mode with canCaptionFinalTts suppresses onPartialReply", async () => {
    setNoAbort();
    ttsMocks.state.synthesizeFinalAudio = true;
    ttsMocks.state.statusSnapshot = {
      autoMode: "tagged",
    ...
    const replyResolver = async (
      ...
      await opts?.onPartialReply?.({
        text: "[[tts:text]]Hello boss[[/tts:text]]",
      });
      await opts?.onBlockReply?.({
        text: "[[tts:text]]Hello boss[[/tts:text]]",
      });
    ...
    const finalPayload = firstFinalReplyPayload(dispatcher);
    expect(finalPayload?.mediaUrl).toBe("https://example.com/tts-synth.opus");
    expect(finalPayload?.text).toBe("Hello boss");
    expect(finalPayload?.audioAsVoice).toBe(true);
  });

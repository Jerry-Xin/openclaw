...
export function createMessageTool(options?: MessageToolOptions): AnyAgentTool {
  ...
  return {
    ...
    execute: async (toolCallId, args, signal) => {
      ...
      // toolCallId + identical params) reproduces the same autogenerated key, which
      // the Gateway dedups to the already-completed "sent" receipt without
      // re-delivering; the cap below uses this key to recognize and admit that replay
      // instead of blocking it. Derivation reads only values fixed above (params,
      ...
      // never truncated; broadcast fan-out is already excluded from budgeting. An
      // idempotent replay of a send already recorded this turn must reach delivery
      // even at the cap: the Gateway resolves the repeated idempotency key to the
      // completed operation and returns "sent" without re-delivering, and
      // recordTurnSendOnce below keeps that replay from double-counting the budget.
      // Blocking it would suppress a receipt the model already earned — the same
      // guard the conversations_send tool applies for its idempotent replays.
